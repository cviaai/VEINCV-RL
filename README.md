...
```
### clDICE loss 
<img src="https://render.githubusercontent.com/render/math?math=L_{clDice}(p, \hat{p})} = 1 - {2 \times \dfrac{T_{prec}(\hat{s}, p) \times T_{sens}(s, \hat{p})}{T_{prec}(\hat{s}, p) + T_{sens}(s, \hat{p})}}">
